...
export default function DiagnoseResult({ reportId }: DiagnoseResultProps) {
  ...
  useEffect(() => {
    const loadReport = async () => {
      setIsLoading(true);
      try {
        // 优先通过 reportId 从服务端获取
        if (reportId) {
          const response = await fetch(`/api/diagnose/report/${reportId}`);
          if (response.ok) {
            const data = await response.json();
            setReport(data);
            // 同时存入 sessionStorage 作为临时缓存（可选）
            sessionStorage.setItem(SS_RESULT, JSON.stringify(data));
            return;
          } else {
            console.warn(`无法通过ID获取报告 (${reportId})，回退到 sessionStorage`);
          }
        }

        // 回退到 sessionStorage
        const storedData = sessionStorage.getItem(SS_RESULT);
        if (storedData) {
          const parsed = JSON.parse(storedData) as DiagnoseReport;
          setReport(parsed);
        }
      } catch (error) {
        console.error("Failed to load diagnose result:", error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleContinueOptimize = (issueIndex?: number) => {
    try {
      const diagnoseData = sessionStorage.getItem(SS_DATA);
      if (diagnoseData) {
        const data = JSON.parse(diagnoseData);
        const issue =
          issueIndex !== undefined
            ? report?.core_issues?.[issueIndex]
            : undefined;
        sessionStorage.setItem(
          ...
          JSON.stringify({
            ...
            diagnoseMode:    data.diagnoseMode || 'basic',
            scrollTo:        issue?.source_location
              ? {
                  paragraph_index: issue.source_location.paragraph_index,
                  text_snippet:    issue.source_location.text_snippet,
                }
              : undefined,
          })
        );
        ...
        <main className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
          <div className="rounded-xl border border-neutral-100 bg-white p-6">
            <div className="flex items-center justify-center">
              ...
              <p className="ml-3 text-sm leading-relaxed text-neutral-600">正在加载诊断结果...</p>
            </div>
          </div>
        </main>
      </div>
  ...
  const isDeepFallback =
    report.metadata?.diagnose_mode === 'deep' &&
    report.metadata?.deep_diagnosis === false &&
    report.metadata?.deep_fallback_message;
        ...
        <div className="grid gap-4 lg:gap-6 lg:grid-cols-12">
          ...
          <section className="space-y-2 lg:col-span-5">
            ...
            <div className="rounded-xl border border-neutral-100 bg-white p-4 sm:p-6 animate-in fade-in slide-in-from-bottom-4 duration-500 break-words">
              {/* Deep fallback banner */}
              {isDeepFallback && (
                <div className="mb-4 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3">
                  <h3 className="text-sm font-semibold text-amber-800">深度诊断暂时不可用</h3>
              ...
              {report.priority_actions?.length > 0 && (
                <div className="mt-4 rounded-lg border border-primary-light bg-gradient-to-r from-primary-light/10 to-transparent px-4 py-3">
                  ...
                  {report.priority_actions[0].description && (
                    <p className="mt-0.5 text-xs leading-relaxed text-neutral-600">{report.priority_actions[0].description}</p>
                  )}
                </div>
              )}
            </div>
